...
export function PrestamosTable({
        ...
        if (localTab !== 'todos' && localTab !== defaultTab) {
            switch (localTab) {
                ...
                case 'visitas_control':
                    relevantPrestamos = relevantPrestamos.filter((p: any) =>
                        parseFloat(p.cobrado_hoy || 0) > 0.01 || (parseFloat(p.cuota_dia_programada || 0) > 0.01 && parseFloat(p.deuda_exigible_hoy || 0) > 0.01)
                    )
                    break
    ...
    const filteredPrestamos = useMemo(() => {
        ...
        switch (activeFilter) {
            ...
            case 'visitas_control':
                ...
                // Además, excluye préstamos programados hoy que pagaron adelantado (deudaHoy 0 y cobrado_hoy 0).
                filtered = filtered.filter(p => (p.cobrado_hoy > 0.01 || (p.cuota_dia_programada > 0.01 && p.deudaHoy > 0.01)) && !['finalizado', 'liquidado', 'anulado', 'castigado', 'renovado', 'refinanciado'].includes(p.estado))
                break
        ...
        filtered.sort((a, b) => {
            ...
            if (activeFilter === 'visitas_control') {
                const isProgA = a.cuota_dia_programada > 0.01 ? 1 : 0
                const isProgB = b.cuota_dia_programada > 0.01 ? 1 : 0
                if (isProgA !== isProgB) return isProgB - isProgA
    ...
    const filterCounts = useMemo(() => {
        ...
        processedPrestamos.forEach(p => {
            ...
            if (!isExcludedEstado) {
                if ((p.cuota_dia_hoy > 0.01 || p.cuota_dia_programada > 0.01) && p.deudaHoy > 0.01) counts.ruta_hoy++
                if (p.cobrado_hoy > 0.01 || (p.cuota_dia_programada > 0.01 && p.deudaHoy > 0.01)) counts.visitas_control++
            }
                                                        ...
                                                        <div className="grid grid-cols-4 gap-1 py-1.5 border-y border-slate-800/40">
                                                            ...
                                                            <div className="flex flex-col items-center border-x border-slate-800/40">
                                                                ...
                                                                <span className={cn(
                                                                    ...
                                                                    prestamo.cobrado_hoy > 0 ? "text-emerald-400" : "text-rose-500/40"
                                                                )}>
                                                                    S/ {(prestamo.cobrado_hoy || 0).toFixed(2)}
                                                                </span>
                                                            </div>
                                                            ...
                                                            <div className="flex flex-col items-center">
                                                                ...
                                                                <div className="h-3 flex items-center justify-center">
                                                                    {prestamo.cobrado_hoy > 0 ? (
                                                                        prestamo.hasVoucherHoy ? (
                                                    ...
                                                    <Fragment key={prestamo.id}>
                                                        ...
                                                        >
                                                            ...
                                                            <div className="col-span-1 text-center">
                                                                {prestamo.cobrado_hoy > 0 ? (
                                                                    <div className="flex flex-col items-center">
                                                                        <span className="text-sm font-black text-emerald-400">S/ {prestamo.cobrado_hoy.toFixed(2)}</span>
                                                                    </div>
                                                                ) : (
                                                            ...
                                                            {/* Recibo / Voucher */}
                                                            <div className="col-span-1 flex justify-center">
                                                                {prestamo.cobrado_hoy > 0 || (isMigrado && isPaid) ? (
                                                                    (prestamo.hasVoucherHoy || isMigrado) ? (
                                                                        <div className="p-1 rounded-full bg-emerald-500/10 border border-emerald-500/20" title="Auditoría OK">
