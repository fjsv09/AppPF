...
import { Landmark, Smartphone, Receipt, CheckCircle2, Clock, Wallet } from 'lucide-react'
...
export function CuadreForm({ carteras, userId }: CuadreFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (Math.abs(totalEntregar - stats.neto) > 1) {
       const confirm = window.confirm(`El monto total a entregar (S/ ${totalEntregar.toFixed(2)}) no coincide exactamente con el neto calculado (S/ ${stats.neto.toFixed(2)}). ¿Desea continuar?`)
       if (!confirm) return
    }
    ...
    try {
      const response = await fetch('/api/cuadres/solicitar', {
        ...
        body: JSON.stringify({
          ...
          p_tipo_cuadre: values.tipo_cuadre
        })
      })
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex sm:block justify-end">
                <span className={`text-[9px] md:text-[10px] px-2 py-1 rounded-full font-bold uppercase tracking-widest ${form.watch('tipo_cuadre') === 'final' ? 'bg-rose-500/20 text-rose-400' : 'bg-blue-500/20 text-blue-400'}`}>
                    {form.watch('tipo_cuadre') === 'final' ? 'Cierre de Día' : 'Cierre Parcial'}
                </span>
            </div>
        </div>
      </CardHeader>
                  ...
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent className="bg-slate-950 border-slate-800 text-white">
                        ...
                        <SelectItem value="final">Cierre de Día (7:00 PM)</SelectItem>
                      </SelectContent>
                    </Select>
                    <FormMessage />
                  </FormItem>
